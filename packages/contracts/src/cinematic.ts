...
import { MAX_CONSISTENCY_REFERENCE_TEXT_CHARS } from "./reference-image.js";
...
export const CinematicSceneSchema = z.object({
  ...
  visualPrompt: z.string().trim().min(1).max(
    PRODUCTION_PROMPT_MAX_CHARACTERS.scene_visual,
  ),
...
const CINEMATIC_CONSISTENCY_REFERENCE_PRIORITY = [
  "character", "product", "element", "environment", "style",
] as const;
...
export const CinematicConsistencyReferenceArtifactSchema = z.object({
  ...
  groups: z.array(z.object({
    ...
    canonicalDescription: z.string().trim().min(1).max(MAX_CONSISTENCY_REFERENCE_TEXT_CHARS),
    prompt: z.string().trim().min(1).max(
      PRODUCTION_PROMPT_MAX_CHARACTERS.consistency_reference,
    ),
    aspectRatio: z.enum(["16:9", "9:16", "1:1"]),
    estimatedCostUsd: z.number().min(0).max(1_000),
    sourceReferenceImageIds: z.array(z.string().uuid()).max(1).default([]),
  }).strict()).max(12),
...
export const CinematicAssetManifestSchema = z.object({
  assets: z.array(z.object({
    ...
    prompt: z.string().trim().min(1).max(
      PRODUCTION_PROMPT_MAX_CHARACTERS.asset_generation,
    ),
    estimatedCostUsd: z.number().min(0).max(1_000),
