...
const WORKFLOW_REENTRY = /(?:回到|返回|切回|切换回|恢复|继续|接着).{0,60}(?:原来|之前|刚才|当前)?(?:的)?.{0,20}(?:工作流|视频流程|视频任务|视频|短片|影片|动画|方案|脚本|分镜|场景规划|素材规划|剪辑方案)/u;
const REVISION_DETAIL = /(?:改成|改为|换成|替换成|替换为|删掉|删除|去掉|移除|增加|添加|补充|减少|缩短|延长|加强|减弱|降低|提高)/u;
...
export const classifyWorkflowReviewInput = (
  ...
  return WORKFLOW_REENTRY.test(normalized) || REVISION_DETAIL.test(normalized)
    ? "revise"
    : "chat";
};
