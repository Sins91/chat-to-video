...
export function StudioShell({ children }: { readonly children: ReactNode }) {
  ...
  if (isFilm) {
    return <div className="grid h-dvh min-h-0 grid-rows-[56px_minmax(0,1fr)] overflow-hidden bg-[#f7f7f8]"><StudioUserBar /><main className="studio-film-frame min-h-0">{children}</main></div>;
  }

  return (
    <div className={`grid h-dvh overflow-hidden bg-[#f7f7f8] ${collapsed ? "grid-cols-[72px_1fr]" : "grid-cols-[232px_1fr]"}`}>
      <aside className="sticky top-0 flex h-dvh flex-col border-r bg-white px-3 py-4">
        <div className={`mb-6 flex h-10 items-center ${collapsed ? "justify-center" : "px-2"}`}>
          <Link href="/" className="flex items-center gap-2 font-bold"><span className="grid size-8 shrink-0 place-items-center rounded-md bg-black text-white"><Clapperboard className="size-4" /></span>{collapsed ? null : <span>FilFil Studio</span>}</Link>
        </div>
        <nav className="space-y-1">
          {navigation.map(({ href, label, icon: Icon }) => {
            const active = href === "/studio" ? pathname === href : pathname.startsWith(href);
            return <Link title={collapsed ? label : undefined} key={href} href={href} className={`flex h-10 items-center gap-3 rounded-md px-3 text-sm ${active ? "bg-black font-medium text-white" : "text-zinc-600 hover:bg-zinc-100"}`}><Icon className="size-4 shrink-0" />{collapsed ? null : label}</Link>;
          })}
        </nav>
...
function StudioUserBar() {
  return <header className="relative z-30 flex h-14 items-center border-b bg-white px-4 sm:px-6">
