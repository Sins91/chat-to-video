...
type PromptInputMenuSelectEvent = Parameters<
...
export const PromptInputActionAddAttachments = ({
  ...
  return (
    <DropdownMenuItem
      {...props}
      onSelect={(event) => void handleSelect(event)}
    >
...
export const PromptInputActionAddScreenshot = ({
  ...
  const handleSelect = useCallback(
    async (event: PromptInputMenuSelectEvent) => {
      onSelect?.(event);
      ...
      try {
        ...
        const convertedFiles: FileUIPart[] = await Promise.all(
          files.map(async (attachment) => {
            const item: FileUIPart & { id?: string } = { ...attachment };
            delete item.id;
  ...
  const inner = (
    <>
      ...
      <form
        ...
        {...props}
      >
        <InputGroup className="overflow-hidden" hasFocusDecoration={false}>
          {children}
        </InputGroup>
      </form>
    </>
