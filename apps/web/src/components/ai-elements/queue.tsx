...
export type QueueItemIndicatorProps = ComponentProps<"span"> & { completed?: boolean };
...
export const QueueItemAction = ({ className, ...props }: QueueItemActionProps) => (
  <Button
    className={cn(
      "size-auto rounded p-1 text-muted-foreground opacity-0 transition-opacity hover:bg-muted-foreground/10 hover:text-foreground group-focus-within:opacity-100 group-hover:opacity-100",
      className,
    )}
    size="icon"
    type="button"
...
export const QueueItemImage = ({ className, ...props }: QueueItemImageProps) => (
  <img alt="" className={cn("size-8 rounded border object-cover", className)} height={32} width={32} {...props} />
);
...
export const Queue = ({ className, ...props }: QueueProps) => (
  <div className={cn("flex flex-col gap-2 rounded-xl border border-border bg-background px-3 pb-2 pt-2 shadow-xs", className)} {...props} />
);
